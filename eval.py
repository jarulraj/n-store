...
COLOR_MAP = ( '#db5375', '#b7999c', '#aaaaaa', '#7293a0', '#dfbe99', '#66586f' )
...
OPT_LINE_COLORS = OPT_COLORS #('#fdc086', '#b3e2cd', '#fc8d62', '#a6cee3', '#e41a1c')
OPT_LINE_WIDTH = 6.0
OPT_MARKER_SIZE = 10.0
DATA_LABELS = []

OPT_STACK_COLORS = OPT_COLORS
# ('#AFAFAF', '#F15854', '#5DA5DA', '#60BD68',  '#B276B2', '#DECF3F', '#F17CB0', '#B2912F', '#FAA43A')
...
def create_nvm_bw_chart(datasets):
    ...
    ax1.set_ylabel(get_label("Bandwidth (MB/s)"), fontproperties=LABEL_FP)
...
def create_ycsb_perf_bar_chart(datasets):
    ...
    ax1.set_ylabel(get_label("Throughput (txn/sec)"), fontproperties=LABEL_FP)
...
def create_ycsb_stack_bar_chart(datasets, abs_time):
    ...
    if abs_mode:
        ax1.set_ylabel(get_label("Absolute Time (s)"), fontproperties=LABEL_FP)
    else:
        ax1.set_ylabel(get_label("Time (\%)"), fontproperties=LABEL_FP)
...
def create_ycsb_storage_bar_chart(datasets):
    ...
    ax1.set_ylabel(get_label("Storage (GB)"), fontproperties=LABEL_FP)
    ax1.yaxis.set_major_locator(MaxNLocator(5))
    axes.set_ylim(0,YLIMIT)

    # X-AXIS
    #ax1.set_xlabel(get_label("Engine"), fontproperties=LABEL_FP)
...
#           bars[group * 2 + 1] = ax1.bar(ind + margin + (group * width), s_misses, width, bottom=l_misses, color=COLOR_MAP[group], hatch=OPT_PATTERNS[group * 2 + 1], linewidth=BAR_LINEWIDTH)
    ...
    if graph_type == 0:
        ax1.set_ylabel(get_label("NVM Loads (M)"), fontproperties=LABEL_FP)
    else:
        ax1.set_ylabel(get_label("NVM Stores (M)"), fontproperties=LABEL_FP)
...
def create_ycsb_recovery_bar_chart(datasets):
    ...
    # Y-AXIS
    ax1.set_ylabel(get_label("Recovery Latency (ms)"), fontproperties=LABEL_FP)
    ax1.set_yscale('log', nonposy='clip')
    ax1.yaxis.set_major_locator(LogLocator(base = 100.0))
    ax1.minorticks_off()

    # X-AXIS
    ax1.set_xlabel(get_label("Number of transactions"), fontproperties=LABEL_FP)
...
def create_tpcc_perf_bar_chart(datasets):
    ...
    # X-AXIS
    #ax1.set_xlabel(get_label("Workload"), fontproperties=LABEL_FP)
    ax1.minorticks_on()
    ax1.set_xticklabels(x_labels)
    ax1.set_xticks(ind + 0.5)
    ax1.tick_params(axis='x', which='both', bottom='off', top='off')
...
def create_btree_line_chart(datasets, sy):
    ...
    ax1.set_xticks(ind + 0.5)
    ax1.set_xscale('log', basex=2)
    ax1.set_xlabel(get_label("B+tree node size (bytes)"), fontproperties=LABEL_FP)
    #ax1.tick_params(axis='x', which='both', bottom='off', top='off')
...
def create_ise_line_chart(datasets, sy):
    ...
    ax1.set_xlabel(get_label("Sync primitive latency (ns)"), fontproperties=LABEL_FP)
    ax1.tick_params(axis='x', which='both', bottom='off', top='off')
