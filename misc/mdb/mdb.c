...
typedef struct MDB_dhead { /* a dirty page */
  SIMPLEQ_ENTRY(MDB_dpage)
  md_next; /* queue of dirty pages */
  MDB_page *md_parent;
...
static MDB_dpage *mdb_newpage(MDB_txn *txn, MDB_page *parent,
                              unsigned int parent_idx, int num);
static int mdb_touch(MDB_txn *txn, MDB_pageparent *mp);

typedef struct MDB_ppage { /* ordered list of pages */
  SLIST_ENTRY(MDB_ppage)
  mp_entry;
  MDB_page *mp_page;
...
static int _mdb_cmp(MDB_db *db, const MDB_val *key1, const MDB_val *key2) {
  if (F_ISSET(db->md_flags, MDB_REVERSEKEY))
    ...
    return memncmp((char *) key1->mv_data, key1->mv_size, key2->mv_data,
                   key2->mv_size);
}
...
mdb_newpage(MDB_txn *txn, MDB_page *parent, unsigned int parent_idx, int num) {
  ...
  if ((dp = malloc(
      txn->mt_env->me_meta.mm_stat.ms_psize * num + sizeof(MDB_dhead))) == NULL)
    return NULL;
  dp->h.md_num = num;
...
int mdb_txn_begin(MDB_env *env, int rdonly, MDB_txn **ret) {
  ...
  DPRINTF("begin transaction %lu on mdbenv %p, root page %lu", txn->mt_txnid,
          (void * ) env, txn->mt_root);
...
void mdb_txn_abort(MDB_txn *txn) {
  ...
  env = txn->mt_env;
  DPRINTF("abort transaction %lu on mdbenv %p, root page %lu", txn->mt_txnid,
          (void * ) env, txn->mt_root);
  ...
  if (F_ISSET(txn->mt_flags, MDB_TXN_RDONLY)) {
    ...
    if (!MDB_IDL_IS_ZERO(txn->mt_free_pgs)) {
      ...
      mop = malloc(
          sizeof(MDB_oldpages) + MDB_IDL_SIZEOF(txn->mt_free_pgs)
              - sizeof(pgno_t));
...
int mdb_txn_commit(MDB_txn *txn) {
  ...
  do {
    ...
    SIMPLEQ_FOREACH(dp, txn->mt_u.dirty_queue, h.md_next)
    {
      if (dp->p.mp_pgno != next) {
        ...
        lseek(env->me_fd, dp->p.mp_pgno * env->me_meta.mm_stat.ms_psize,
        SEEK_SET);
        next = dp->p.mp_pgno;
      }
  ...
  if ((n = mdbenv_sync(env)) != 0 || (n = mdbenv_write_meta(txn)) != MDB_SUCCESS
      || (n = mdbenv_sync(env)) != 0) {
    mdb_txn_abort(txn);
    return n;
  }
...
static int mdbenv_init_meta(MDB_env *env) {
  MDB_page *p, *q;
  MDB_meta *meta;
  int rc;
  unsigned int psize;
...
int mdbenv_open2(MDB_env *env, unsigned int flags) {
  ...
  DPRINTF("opened database version %u, pagesize %u", env->me_meta.mm_version,
          env->me_meta.mm_stat.ms_psize);
 ...
 */
static MDB_node *
mdb_search_node(MDB_db *bt, MDB_page *mp, MDB_val *key, int *exactp,
                unsigned int *kip) {
  unsigned int i = 0;
  int low, high;
  int rc = 0;
  ...
  DPRINTF("searching %u keys in %s page %lu", NUMKEYS(mp),
  ...
  while (low <= high) {
    ...
    if (IS_LEAF(mp))
      DPRINTF("found leaf index %u [%.*s], rc = %i", i, (int )nodekey.mv_size,
              (char * )nodekey.mv_data, rc);
    else
      DPRINTF("found branch index %u [%.*s -> %lu], rc = %i", i,
              (int )node->mn_ksize, (char *)NODEKEY(node), node->mn_pgno, rc);
...
static void cursor_pop_page(MDB_cursor *cursor) {
  ...
  DPRINTF("popped page %lu off cursor %p", top->mp_page->mp_pgno,
          (void * ) cursor);
...
static int mdb_search_page_root(MDB_db *bt, MDB_val *key, MDB_cursor *cursor,
  ...
  while (IS_BRANCH(mp)) {
    ...
    if (key)
      DPRINTF("following index %u for key %.*s", i, (int )key->mv_size,
              (char * )key->mv_data);
    assert(i < NUMKEYS(mp));
    node = NODEPTR(mp, i);
...
static int mdb_sibling(MDB_cursor *cursor, int move_right) {
  ...
  if (move_right ?
      (parent->mp_ki + 1 >= NUMKEYS(parent->mp_page)) : (parent->mp_ki == 0)) {
...
static int mdb_cursor_set(MDB_cursor *cursor, MDB_val *key, MDB_val *data,
  ...
  if (data
      && (rc = mdb_read_data(cursor->mc_db, mpp.mp_page, leaf, data))
          != MDB_SUCCESS)
    return rc;
...
mdbenv_new_page(MDB_env *env, uint32_t flags, int num) {
  ...
  if ((dp = mdb_newpage(env->me_txn, NULL, 0, num)) == NULL)
    return NULL;
  DPRINTF("allocated new mpage %lu, page size %u", dp->p.mp_pgno,
          env->me_meta.mm_stat.ms_psize);
...
static int mdb_add_node(MDB_db *db, MDB_page *mp, indx_t indx, MDB_val *key,
  ...
  if (IS_LEAF(mp)) {
    ...
    if (F_ISSET(flags, F_BIGDATA)) {
      ...
      node_size += sizeof(pgno_t);
        ...
        >= db->md_env->me_meta.mm_stat.ms_psize / MDB_MINKEYS) {
      int ovpages = OVPAGES(data->mv_size, db->md_env->me_meta.mm_stat.ms_psize)
      ;
      /* Put data on overflow page. */
      DPRINTF("data size is %zu, put on overflow page", data->mv_size);
      node_size += sizeof(pgno_t);
  ...
  if (node_size + sizeof(indx_t) > SIZELEFT(mp)) {
    DPRINTF("not enough room in page %lu, got %u ptrs", mp->mp_pgno,
            NUMKEYS(mp));
  ...
  if (IS_LEAF(mp)) {
    ...
    if (ofp == NULL) {
      if (F_ISSET(flags, F_BIGDATA))
        bcopy(data->mv_data, node->mn_data + key->mv_size, sizeof(pgno_t));
      else
        bcopy(data->mv_data, node->mn_data + key->mv_size, data->mv_size);
    } else {
      bcopy(&ofp->p.mp_pgno, node->mn_data + key->mv_size, sizeof(pgno_t));
...
static int mdb_update_key(MDB_db *db, MDB_page *mp, indx_t indx, MDB_val *key) {
  ...
  node = NODEPTR(mp, indx);
  ptr = mp->mp_ptrs[indx];
  DPRINTF("update key %u (ofs %u) [%.*s] to [%.*s] on page %lu", indx, ptr,
          (int )node->mn_ksize, (char *)NODEKEY(node), (int )key->mv_size,
          (char * )key->mv_data, mp->mp_pgno);
...
static int mdb_move_node(MDB_db *bt, MDB_pageparent *src, indx_t srcindx,
  ...
  if (srcindx == 0 && src->mp_pi != 0) {
    ...
    if ((rc = mdb_update_key(bt, src->mp_parent, src->mp_pi, &key))
        != MDB_SUCCESS)
      return rc;
  }
  ...
  if (dstindx == 0 && dst->mp_pi != 0) {
    ...
    if ((rc = mdb_update_key(bt, dst->mp_parent, dst->mp_pi, &key))
        != MDB_SUCCESS)
      return rc;
  }
...
static int mdb_merge(MDB_db *bt, MDB_pageparent *src, MDB_pageparent *dst) {
  ...
  for (i = 0; i < NUMKEYS(src->mp_page); i++) {
    ...
    data.mv_size = NODEDSZ(srcnode);
    data.mv_data = NODEDATA(srcnode);
    rc = mdb_add_node(bt, dst->mp_page, NUMKEYS(dst->mp_page), &key, &data,
                      NODEPGNO(srcnode), srcnode->mn_flags);
    if (rc != MDB_SUCCESS)
      return rc;
  }

  DPRINTF("dst page %lu now has %u keys (%.1f%% filled)", dst->mp_page->mp_pgno,
          NUMKEYS(dst->mp_page),
...
static int mdb_rebalance(MDB_db *db, MDB_pageparent *mpp) {
  ...
  DPRINTF("rebalancing %s page %lu (has %u keys, %.1f%% full)",
          IS_LEAF(mpp->mp_page) ? "leaf" : "branch", mpp->mp_page->mp_pgno,
          NUMKEYS(mpp->mp_page),
  ...
  if (mpp->mp_parent == NULL) {
    if (NUMKEYS(mpp->mp_page) == 0) {
      ...
      if ((root = mdbenv_get_page(db->md_env, db->md_env->me_txn->mt_root))
          == NULL)
        return MDB_FAIL;
      db->md_env->me_meta.mm_stat.ms_depth--;
...
static int mdb_split(MDB_db *bt, MDB_page **mpp, unsigned int *newindxp,
  ...
  if (SIZELEFT(rdp->h.md_parent) < mdb_branch_size(bt, &sepkey)) {
    rc = mdb_split(bt, &rdp->h.md_parent, &rdp->h.md_pi, &sepkey, NULL,
                   rdp->p.mp_pgno);
    ...
    rc = mdb_add_node(bt, rdp->h.md_parent, rdp->h.md_pi, &sepkey, NULL,
                      rdp->p.mp_pgno, 0);
  }
  if (rc != MDB_SUCCESS) {
    free(copy);
